  ...
  return (
    <div className="min-h-screen bg-cream max-w-2xl mx-auto">
      <div className="px-5 pt-4 pb-8">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            <div className="font-display font-normal italic text-lg tracking-tight">
              ...
              <span className="font-mono text-[9px] text-ink-3 tracking-wider uppercase ml-2 not-italic">Admin</span>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <Link
              href="/admin"
              className="font-mono text-[9px] tracking-wider uppercase text-ink-3 hover:text-ink"
            >
              Tenants
            </Link>
            <button
              onClick={handleLogout}
              className="font-mono text-[9px] tracking-wider uppercase text-warn"
            >
              Sign out
            </button>
          </div>
        ...
        {loading || !stats ? (
          <div className="text-center py-12 text-ink-3 font-mono text-xs">Loading stats...</div>
        ) : (
          <>
            {/* Platform KPIs */}
            <div className="grid grid-cols-4 gap-2 mb-6">
              <div className="bg-paper rounded-card p-3">
                <div className="font-mono text-[7px] tracking-wider uppercase text-ink-3 mb-1">Tenants</div>
                <div className="font-display text-2xl leading-none">{stats.totals.tenants}</div>
              </div>
              <div className="bg-paper rounded-card p-3">
                <div className="font-mono text-[7px] tracking-wider uppercase text-ink-3 mb-1">Bookings</div>
                <div className="font-display text-2xl leading-none">{stats.totals.bookings}</div>
              </div>
              <div className="bg-paper rounded-card p-3">
                <div className="font-mono text-[7px] tracking-wider uppercase text-ink-3 mb-1">Members</div>
                <div className="font-display text-2xl leading-none">{stats.totals.members}</div>
              </div>
              <div className="bg-paper rounded-card p-3">
                <div className="font-mono text-[7px] tracking-wider uppercase text-ink-3 mb-1">Users</div>
                <div className="font-display text-2xl leading-none">{stats.totals.users}</div>
              </div>
            </div>
            ...
            <div className="mb-6">
              ...
              <div className="bg-paper rounded-card p-4">
                <svg viewBox="0 0 280 70" className="w-full overflow-visible">
                  ...
                  {sparkPath && (
                    <path d={sparkPath} fill="none" stroke="#1A1715" strokeWidth="1.4" strokeLinejoin="round" strokeLinecap="round" />
                  )}
                </svg>
              </div>
            </div>
            ...
            <div className="mb-6">
              ...
              <div className="bg-paper rounded-card p-4 space-y-2">
                  ...
                  return (
                    <div key={status}>
                      <div className="flex justify-between mb-1">
                        <span className="font-mono text-[9px] tracking-wider uppercase text-ink-2">{status.replace('_', ' ')}</span>
                        <span className="font-mono text-[9px] text-ink-3">{count} ({pct}%)</span>
                      </div>
                    ...
                    {monthLabels.map((month, i) => {
                      ...
                      return (
                        <div key={month} className="flex-1 flex flex-col items-center gap-1">
                          <div className="font-mono text-[8px] text-ink-3">{val}</div>
                          <div
                            className="w-full bg-accent rounded-sm"
                            style={{ height: `${Math.max(heightPct, 4)}%` }}
                          />
                          <div className="font-mono text-[7px] text-ink-3 tracking-wider uppercase">
                            {month.slice(5)}
                          </div>
                        </div>
                      );
